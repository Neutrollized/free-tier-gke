...
#
# TO RUN: source ./wi-utils.sh && [FUNCTION] [ARG1] [ARG2] ...
# i.e. source ./wi-utils.sh && verify_workload_identity_binding kf-ksa kubeflow
...
function verify_workload_identity_binding {
  ...
  do
    ...
    kubectl run test-$RANDOM --rm -i --restart=Never \
        ...
        --overrides='{ "spec": { "serviceAccountName": "'$ksa'" } }' \
        --namespace $namespace \
        -- gsutil ls gs://$gcs_bucket || workload_identity_is_ready=false
